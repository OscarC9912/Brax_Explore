...
class AllPairs(Cull):
  ...
  def __init__(self, col_a: geometry.Collidable, col_b: geometry.Collidable,
    ...
    self.col_a = col_a.take([a for a, _ in mask])
    self.col_b = col_b.take([b for _, b in mask])
...
class NearNeighbors(Cull):
  ...
  def __init__(self, col_a: geometry.Collidable, col_b: geometry.Collidable,
    ...
    dist_off = np.zeros(col_a.body.idx.shape + col_b.body.idx.shape)
    # TODO: consider accounting for bounds/radius of a collidable
    dist_mask = dist_off + float('inf')
    mask = (np.array([a for a, _ in mask]), np.array([b for _, b in mask]))
    dist_off += arrop.index_update(dist_mask, mask, 0)
  ...
  def update(self, qp: QP):
    if self.candidate_a is self.candidate_b:
      ...
      dist = arrop.torchVmap(lambda pt: arrop.norm(pos - pt, axis=-1))(pos)
    else:
      pos_a = self.candidate_a.position(qp)
      pos_b = self.candidate_b.position(qp)
      dist = arrop.torchVmap(lambda pt: arrop.norm(pos_b - pt, axis=-1))(pos_a)
    # add in offset and convert to closeness
    sim = -(dist + self.dist_off)
    # TODO: return a status if more valid candidates than cutoff
    _, idx = arrop.top_k(sim.ravel(), self.cutoff)
    self.col_a = arrop.take(self.candidate_a, idx // sim.shape[-1])
    self.col_b = arrop.take(self.candidate_b, idx % sim.shape[-1])
...
class Collider(abc.ABC):
  ...
  def __init__(self, contact_fn: Callable[[Any, Any, QP, QP], geometry.Contact],
               cull: Cull, config: torch_config_pb2.Config):
    ...
    self.velocity_threshold = arrop.norm(vec_to_arr(config.gravity)) * self.h * 4.0
  ...
  def apply(self, qp: QP) -> P:
    ...
    qp_a = arrop.take(qp, col_a.body.idx)
    qp_b = arrop.take(qp, col_b.body.idx)
    contact = arrop.torchVmap(self.contact_fn)(col_a, col_b, qp_a, qp_b)
    dp_a, dp_b = arrop.torchVmap(self._contact)(col_a, col_b, qp_a, qp_b, contact)

    if dp_b is None:
      dp_vel, dp_ang, body_idx = dp_a.vel, dp_a.ang, col_a.body.idx
    else:
      body_idx = arrop.concatenate((col_a.body.idx, col_b.body.idx))
      dp_vel = arrop.concatenate((dp_a.vel, dp_b.vel))
      dp_ang = arrop.concatenate((dp_a.ang, dp_b.ang))
    contact = arrop.where(np.any(dp_vel, axis=-1), 1.0, 0.0)
    contact = arrop.segment_sum(contact, body_idx, qp.pos.shape[0])
    ...
    contact = arrop.reshape(1e-8 + contact, (dp_vel.shape[0], 1))
    dp_vel = dp_vel / contact
    dp_ang = dp_ang / contact
    return P(vel=dp_vel, ang=dp_ang)

  def velocity_apply(self, qp: QP, dlambda: np.ndarray, qp_prev: QP,
                     contact) -> P:
    """Returns impulse from any potential contacts between collidables.
    ...
    """
    col_a, col_b = self.cull.get()
    qp_a, qp_a_prev = arrop.take((qp, qp_prev), col_a.body.idx)
    qp_b, qp_b_prev = arrop.take((qp, qp_prev), col_b.body.idx)
    dp_a, dp_b = arrop.vmap(self._velocity_contact)(
        col_a,
        col_b,
        qp_a,
    ...
    # TODO: modify the condition here to normal torch script
    contact = arrop.where(np.any(dp_vel, axis=-1), 1.0, 0.0)
    ...
    # equally distribute impulse over possible contacts
    contact = arrop.reshape(1e-6 + contact, (dp_vel.shape[0], 1))
    dp_vel = dp_vel / contact
    dp_ang = dp_ang / contact
    return P(vel=dp_vel, ang=dp_ang)

  def position_apply(self, qp: QP,
                     qp_prev: QP) -> Tuple[Q, np.ndarray, geometry.Contact]:
    ...
    contact = arrop.torchVmap(self.contact_fn)(col_a, col_b, qp_a, qp_b)
    pre_contact = contact
    dq_a, dq_b, dlambda = arrop.torchVmap(self._position_contact)(col_a, col_b, qp_a,
                                                          qp_b, qp_a_prev,
                                                          qp_b_prev, contact)
    ...
    if dq_b is None:
      ...
      body_idx = arrop.concatenate((col_a.body.idx, col_b.body.idx))
      dq_pos = arrop.concatenate((dq_a.pos, dq_b.pos))
      dq_rot = arrop.concatenate((dq_a.rot, dq_b.rot))
    contact = arrop.where(np.any(dq_pos, axis=-1), 1.0, 0.0)
    contact = arrop.segment_sum(contact, body_idx, qp.pos.shape[0])
    dq_pos = arrop.segment_sum(dq_pos, body_idx, qp.pos.shape[0])
    dq_rot = arrop.segment_sum(dq_rot, body_idx, qp.rot.shape[0])

    # equally distribute impulse over possible contacts
    contact = arrop.reshape(1e-6 + contact, (dq_pos.shape[0], 1))
  ...
  def _position_contact(
      ...
      contact: geometry.Contact) -> Tuple[Q, Optional[Q], np.ndarray]:
    pass

  @abc.abstractmethod
  def _velocity_contact(self, col_a: geometry.Collidable,
                        col_b: geometry.Collidable, qp_a: QP, qp_b: QP,
                        contact: geometry.Contact, dlambda: np.ndarray,
...
class OneWayCollider(Collider):
  ...
  def _contact(self, col_a: geometry.Collidable, col_b: geometry.Collidable,
    ...
    normal_vel = arrop.dot(contact.normal, contact.vel)
    temp1 = col_a.body.inertia * arrop.cross(rel_pos, contact.normal)
    ang = arrop.dot(contact.normal, arrop.cross(temp1, rel_pos))
    ...
    impulse_d = arrop.safe_norm(vel_d) / ((1. / (col_a.body.mass)) + ang)
    # drag magnitude cannot exceed max friction
    impulse_d = arrop.minimum(impulse_d, friction * impulse)
    dir_d = vel_d / (1e-6 + arrop.safe_norm(vel_d))
    ...
    apply_d = apply_n * arrop.where(arrop.norm(vel_d) > 0.01, 1., 0.)
  ...
  def _position_contact(
    ...
    cr1 = arrop.cross(pos_p, n)
    w1 = (1. / col_a.body.mass) + arrop.dot(cr1, col_a.body.inertia * cr1)

    dlambda = -c / (w1 + 1e-6)

    coll_mask = arrop.where(c < 0, 1., 0.)
    p = dlambda * n * coll_mask

    dq_p_pos = p / col_a.body.mass
    dq_p_rot = .5 * math.vec_quat_mul(col_a.body.inertia * arrop.cross(pos_p, p),
                                      qp_a.rot)
    ...
    deltap = p1 - p1bar
    deltap_t = deltap - arrop.dot(deltap, n) * n

    dx = deltap_t

    c = arrop.safe_norm(dx)
    n = dx / (c + 1e-6)
    ...
    # TODO: np.abs to be implemented
    static_mask = arrop.where(
        np.abs(dlambdat) < np.abs(friction * dlambda), 1., 0.)
  ...
  def _velocity_contact(self, col_a: geometry.Collidable,
    ...
    n = contact.normal
    rel_vel = qp_a.vel + arrop.cross(qp_a.ang, contact.pos - qp_a.pos)
    v_n = arrop.dot(rel_vel, n)
    v_t = rel_vel - n * v_n
    v_t_norm = arrop.safe_norm(v_t)
    v_t_dir = v_t / (1e-6 + v_t_norm)

    # factor of 2 from integrator doing 1 collision pass every 2 steps
    dvel = -v_t_dir * arrop.amin(
        arrop.array([friction * np.abs(dlambda) / (2. * self.h), v_t_norm]))

    angw = arrop.cross((contact.pos - qp_a.pos), v_t_dir)
    w = (1. / col_a.body.mass) + arrop.dot(angw, angw)

    p_dyn = dvel / (w + 1e-6)

    # restitution calculation

    rel_vel_old = qp_a_old.vel + arrop.cross(qp_a_old.ang,
                                          contact.pos - qp_a_old.pos)
    v_n_old = arrop.dot(rel_vel_old, n)

    dv_rest = n * (-v_n - arrop.amin(arrop.array([elasticity * v_n_old, 0.])))
    ...
    static_mask = arrop.where(contact.penetration > 0, 1., 0.)
    sinking = arrop.where(v_n_old <= -self.velocity_threshold, 1., 0.)

    p = (dlambda_rest * n * sinking + p_dyn) * static_mask

    dp_p = P(
        vel=p / col_a.body.mass,
        ang=arrop.cross(col_a.body.inertia * (contact.pos - qp_a.pos), p))
...
class TwoWayCollider(Collider):
  ...
  def _contact(self, col_a: geometry.Collidable, col_b: geometry.Collidable,
    ...
    baumgarte_vel = self.baumgarte_erp * contact.penetration
    normal_vel = arrop.dot(contact.normal, contact.vel)
    temp1 = col_a.body.inertia * arrop.cross(rel_pos_a, contact.normal)
    temp2 = col_b.body.inertia * arrop.cross(rel_pos_b, contact.normal)
    ang = arrop.dot(contact.normal,
                 arrop.cross(temp1, rel_pos_a) + arrop.cross(temp2, rel_pos_b))
    impulse = (-1. * (1. + elasticity) * normal_vel + baumgarte_vel) / (
    ...
    # apply drag due to friction acting parallel to the surface contact
    vel_d = contact.vel - normal_vel * contact.normal
    impulse_d = arrop.safe_norm(vel_d) / ((1. / col_a.body.mass) +
    ...
    apply_n = arrop.where(
        (contact.penetration > 0.) & (normal_vel < 0) & (impulse > 0.), 1., 0.)
    # apply drag if moving laterally above threshold
    apply_d = apply_n * arrop.where(arrop.safe_norm(vel_d) > 0.01, 1., 0.)
  ...
  def _position_contact(
    ...
    cr2 = arrop.cross(pos_c, n)
    w2 = (1. / col_b.body.mass) + arrop.dot(cr2, col_b.body.inertia * cr2)
    ...
    dq_c_rot = -.5 * math.vec_quat_mul(col_b.body.inertia * arrop.cross(pos_c, p),
                                       qp_b.rot)
    ...
    dlambdat = -c / (w1 + w2)
    static_mask = arrop.where(np.abs(dlambdat) < np.abs(dlambda), 1., 0.)
    p = dlambdat * n * static_mask * coll_mask
  ...
  def _velocity_contact(self, col_a: geometry.Collidable,
    ...
    n = contact.normal
    rel_vel = qp_a.vel + arrop.cross(qp_a.ang, contact.pos - qp_a.pos) - (
        qp_b.vel + arrop.cross(qp_b.ang, contact.pos - qp_b.pos))
    v_n = arrop.dot(rel_vel, n)
    v_t = rel_vel - n * v_n
    v_t_norm = arrop.safe_norm(v_t)
    v_t_dir = v_t / (1e-6 + v_t_norm)
    ...
    angw_1 = arrop.cross((contact.pos - qp_a.pos), v_t_dir)
    angw_2 = arrop.cross((contact.pos - qp_b.pos), v_t_dir)
    w1 = (1. / col_a.body.mass) + arrop.dot(angw_1, col_a.body.inertia * angw_1)
    w2 = (1. / col_b.body.mass) + arrop.dot(angw_2, col_b.body.inertia * angw_2)
    ...
    rel_vel_old = (
        qp_a_old.vel + arrop.cross(qp_a_old.ang, contact.pos - qp_a_old.pos)) - (
            qp_b_old.vel + arrop.cross(qp_b_old.ang, contact.pos - qp_b_old.pos))
    v_n_old = arrop.dot(rel_vel_old, n)
    ...
    static_mask = arrop.where(contact.penetration > 0, 1., 0.)
    sinking = arrop.where(v_n_old <= 0., 1., 0.)
    ...
    dp_c = P(
        vel=-p / col_a.body.mass,
        ang=arrop.cross(col_b.body.inertia * (contact.pos - qp_b.pos), -p))
...
def _endpoints(end: np.ndarray, qp: QP, offset: np.ndarray):
...
def box_plane(box: geometry.BoxCorner, _: geometry.Plane, qp_a: QP,
  ...
  normal = math.rotate(np.array([0., 0., 1.]), qp_b.rot)
  penetration = arrop.dot(qp_b.pos - pos, normal)
...
def box_heightmap(box: geometry.BoxCorner, hm: geometry.HeightMap, qp_a: QP,
  ...
  # two triangles.
  uv_idx = np.floor(uv_pos).astype(int)
  delta_uv = uv_pos - uv_idx
  lower_triangle = np.sum(delta_uv) < 1
  mu = arrop.where(lower_triangle, -1, 1)

  # Compute the triangle vertices (u, v) that enclose the candidate point.
  triangle_u = uv_idx[0] + arrop.where(lower_triangle, np.array([0, 1, 0]),
                                    np.array([1, 0, 1]))
  triangle_v = uv_idx[1] + arrop.where(lower_triangle, np.array([0, 0, 1]),
                                    np.array([1, 1, 0]))
  ...
  raw_normal = np.array([mu * (h[1] - h[0]), mu * (h[2] - h[0]), hm.cell_size])
  normal = raw_normal / arrop.norm(raw_normal)
  p0 = np.array(
      [triangle_u[0] * hm.cell_size, triangle_v[0] * hm.cell_size, h[0]])
  penetration = arrop.dot(p0 - pos, normal)
...
def capsule_plane(cap: geometry.CapsuleEnd, _: geometry.Plane, qp_a: QP,
  ...
  normal = math.rotate(arrop.array([0., 0., 1.]), qp_b.rot)
  pos = cap_end_world - normal * cap.radius
  vel = qp_a.vel + arrop.cross(qp_a.ang, pos - qp_a.pos)
  penetration = arrop.dot(qp_b.pos - pos, normal)
  return geometry.Contact(pos, vel, normal, penetration)
...
def capsule_capsule(cap_a: geometry.Capsule, cap_b: geometry.Capsule, qp_a: QP,
  ...
  dist = arrop.safe_norm(penetration_vec)
...
def get(config: torch_config_pb2.Config, body: bodies.Body) -> List[Collider]:
  ...
  for b in config.bodies:
    for collider in b.colliders:
      ...
      if collider.WhichOneof('type') == 'sphere':
        new_collider = torch_config_pb2.Collider()
      ...
      new_body = torch_config_pb2.Body()
      new_body.CopyFrom(b)
  ...
  for type_a, type_b in itertools.combinations_with_replacement(
    ...
    if col_cls_a not in collidable_cache:
      new_a = create_collidable(col_cls_a, cols_a, b1)
      collidable_cache[col_cls_a] = copy.deepcopy(new_a)

    if col_cls_b not in collidable_cache:
      new_b = create_collidable(col_cls_b, cols_b, b2)
      collidable_cache[col_cls_b] = copy.deepcopy(new_b)
